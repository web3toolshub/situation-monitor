<script lang="ts">
	...
	// D3 objects - typed as unknown with runtime null checks
	...
	function setCachedData<T>(key: string, data: T): void {
		...
		if (Object.keys(dataCache).length >= MAX_CACHE_SIZE) {
			...
			for (const k of Object.keys(dataCache)) {
				if (now - dataCache[k].timestamp > CACHE_TTLS.weather) {
					delete dataCache[k];
				}
			}
			// If still at capacity, remove oldest half
			if (Object.keys(dataCache).length >= MAX_CACHE_SIZE) {
				const sorted = Object.entries(dataCache).sort((a, b) => a[1].timestamp - b[1].timestamp);
				sorted.slice(0, Math.ceil(sorted.length / 2)).forEach(([k]) => delete dataCache[k]);
			}
		}
