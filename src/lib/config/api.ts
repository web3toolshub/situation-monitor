...
/**
 * Fetch with CORS proxy fallback and retry
...
export async function fetchWithProxy(url: string): Promise<Response> {
	...
	try {
		const response = await fetchWithRetry(CORS_PROXIES.primary + encodedUrl);
		if (response.ok) {
			return response;
		}
	...
	return fetchWithRetry(CORS_PROXIES.fallback + encodedUrl);
}
