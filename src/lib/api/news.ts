...
function transformGdeltArticle(
	...
	const urlHash = article.url ? hashString(article.url).toString(36) : Math.random().toString(36).slice(2);
...
/**
 * Fetch all news - parallel with staggered delays to avoid rate limiting
 */
export async function fetchAllNews(): Promise<Record<NewsCategory, NewsItem[]>> {
	// Fetch all categories in parallel
	const promises = NEWS_CATEGORIES.map((category, index) =>
		// Add staggered delay based on index
		delay(index * API_DELAYS.betweenCategories).then(() => fetchCategoryNews(category))
	);
